...
export const EventProvider = ({ children }) => {
  const loadMergedState = (fbData) => {
    const fallbackFbStr = localStorage.getItem('rd_last_known_fb_data');
    let baseData = fbData;
    ...
    if (!baseData && fallbackFbStr) {
        ...
        localStorage.setItem('rd_last_known_fb_data', JSON.stringify(fbData));
    }

    const localStr = localStorage.getItem('rd_local_changes');
  ...
  const saveChangeLocal = (id, objOrNull) => {
    ...
    localStorage.setItem('rd_local_changes', JSON.stringify(localChanges));
  ...
  const syncPendingChangesToCloud = () => {
    ...
    keys.forEach(id => {
      ...
      if (change === null) {
        deleteDoc(doc(db, 'rd_deadline_items', id)).catch(e => saveChangeLocal(id, null));
      } else {
        setDoc(doc(db, 'rd_deadline_items', id), change, { merge: true }).catch(e => saveChangeLocal(id, change));
      }
    });
  };
  ...
  // Sync with Firestore
  useEffect(() => {
    const q = query(collection(db, 'rd_deadline_items'), orderBy('dueDate', 'asc'));
  ...
  const addEvent = async (item) => {
    ...
    try { 
      await setDoc(doc(db, 'rd_deadline_items', newId), newItem); 
  ...
  const updateEvent = async (updated) => {
    ...
    try { 
      await setDoc(doc(db, 'rd_deadline_items', updated.id), updated, { merge: true }); 
  ...
  const deleteEvent = async (id) => {
    ...
    setCurrentEvent(null);
    try { 
      await deleteDoc(doc(db, 'rd_deadline_items', id)); 
      clearChangeLocal(id);
    } catch (e) { console.warn(e); }
  };
  ...
  const changeStatus = async (id, status) => {
    setItems((prev) => {
      ...
      setDoc(doc(db, 'rd_deadline_items', id), upd, { merge: true })
